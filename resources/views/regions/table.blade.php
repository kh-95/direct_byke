...
<div class="table-responsive">
    <table class="table" id="users-table">
        ...
        <tr>
            ...
            <th class="text-center">{{__('models/regions.fields.enabled') }}</th>
        ...
        @foreach($regions as $key => $region)
            <tr>
                ...
                <td class=" text-center">{{ $region->{'name_'.app()->getLocale()} }}</td>
                ...
                @else
                    <td class=" text-center">
                        <img src="{{ asset('img/remove.png') }}"
                             class="shadow-light"></td>
                @endif
                <td class=" text-center">{{ $region->lat }}</td>
                <td class=" text-center">{{ $region->lon }}</td>
...
{{--                        </div>--}}
                        ...
                        <input data-id="{{$region->id}}" class="toggle-class" type="checkbox" data-onstyle="success"
                               data-offstyle="danger" data-toggle="toggle" data-on="{{__('validation.active')}}"
                               data-off="{{__('validation.in_active')}}" {{ !$region->is_active ? 'checked' : '' }}
                        >
                    </div>
                    {!! Form::close() !!}
        ...
        $(function () {
            $('.toggle-class').change(function () {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: "{{url('changeStatusRegion')}}?status=" + status + "&region_id=" + region_id,
